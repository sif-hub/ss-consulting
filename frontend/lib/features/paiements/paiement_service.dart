...
class PaiementService {
  ...
  Future<List<Paiement>> getPaiements({int? factureId}) async {
    final queryParameters = <String, dynamic>{'facture_id': ?factureId};
    ...
    return data
        .map(
          (item) => Paiement.fromJson(Map<String, dynamic>.from(item as Map)),
        )
        .toList();
  }

  Future<Paiement> getPaiement(int id) async {
    final Response response = await _apiClient.dio.get('/paiements/$id');

    return Paiement.fromJson(Map<String, dynamic>.from(response.data as Map));
  }

  Future<Paiement> createPaiement(Map<String, dynamic> data) async {
  ...
  Future<Paiement> initierPaiementMobile(Map<String, dynamic> data) async {
    final Response response = await _apiClient.dio.post(
  ...
  Future<Paiement> confirmerPaiementMobile(int paiementId) async {
    final Response response = await _apiClient.dio.post(
  ...
  /// Taux (en %) ajouté par-dessus le montant payé par Mobile Money.
  Future<double> getTauxCommission() async {
    final Response response = await _apiClient.dio.get('/paiements/commission');

    return ((response.data as Map)['taux'] as num).toDouble();
  }
  ...
  Future<Paiement> updatePaiement(int id, Map<String, dynamic> data) async {
  ...
  Future<void> deletePaiement(int id) async {
    await _apiClient.dio.delete('/paiements/$id');
  }

  Future<Map<String, dynamic>> getResumeFacture(int factureId) async {
    final Response response = await _apiClient.dio.get(
